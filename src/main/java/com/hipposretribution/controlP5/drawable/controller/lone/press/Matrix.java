package com.hipposretribution.controlP5.drawable.controller.lone.press;
...
/**
 * A matrix is a 2d array with a pointer that traverses through the matrix in a timed interval. if an item of
 * a matrix-column is active, the x and y position of the corresponding cell will trigger an event and notify
 * the program. see the ControlP5matrix example for more information.
 * 
 * @example controllers/ControlP5matrix
 */
public class Matrix extends CElement<Matrix> {
	...
	protected int _myMode = ControlP5.SINGLE_ROW;
	...
	public Matrix(ControlP5 theControlP5, CGroup<?> theParent, String theName, int theCellX, int theCellY,
		...
		if (getIsInside()) {
			if (isPressed) {
				...
				if (tX != currentX || tY != currentY) {
					...
					switch (_myMode) {
					default:
					case (ControlP5.SINGLE_COLUMN):
						for (int i = 0; i < _myCellY; i++) {
							_myCells[tX][i] = 0;
						}
						_myCells[tX][tY] = (!isMarkerActive) ? 1 : _myCells[tX][tY];
						break;
					case (ControlP5.SINGLE_ROW):
						for (int i = 0; i < _myCellY; i++) {
							_myCells[tX][i] = 0;
						}
						_myCells[tX][tY] = (!isMarkerActive) ? 1 : _myCells[tX][tY];
						break;
					case (ControlP5.MULTIPLES):
	...
	/**
	 * use setMode to change the cell-activation which by default is ControlP5.SINGLE_ROW, 1 active cell per
	 * row, but can be changed to ControlP5.SINGLE_COLUMN or ControlP5.MULTIPLES
	 * 
	...
	@Override
	@ControlP5.Invisible
	public Matrix updateDisplayMode(ControllerViewType theMode) {
		displayMode = theMode;
		switch (theMode) {
		case DEFAULT:
			controllerView = new MatrixView();
			break;
		case IMAGE:
		case SPRITE:
		case CUSTOM:
