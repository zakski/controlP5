package com.hipposretribution.controlP5.drawable;
...
public class Tooltip {
	...
	private CElement<?> _myController;
	...
	private Map<CElement<?>, String> map;
	...
	public Tooltip(ControlP5 theControlP5) {
		...
		map = new HashMap<CElement<?>, String>();
	...
	public void draw(ControlWindow theWindow) {
		...
		if (enabled) {
			...
			if (_myMode >= ControlP5.WAIT) {
				...
				currentPosition.set(cp5.mouseX(), cp5.mouseY(), 0);
	...
	/**
	 * A tooltip is activated when entered by the mouse, after a given delay time the Tooltip starts to fade
	 * in. Use setDelay(long) to adjust the default delay time of 1000 millis.
	 * 
	 * @param theMillis
	 * @return Tooltip
	 */
	 ...
	 * @param theController
	 */
	public void activate(CElement<?> theController) {
	...
	/**
	 * A custom view can be set for a Tooltip. The default view class can be found at the bottom of the
	 * Tooltip source.
	 * 
	...
	/**
	 * registers a controller with the Tooltip, when activating the tooltip for a particular controller, the
	 * registered text (second parameter) will be displayed.
	 * 
	 * @param theController
	 * @param theText
	 * @return Tooltip
	 */
	public Tooltip register(CElement<?> theController, String theText) {
		map.put(theController, theText);
		return this;
	}

	public Tooltip register(String theControllerName, String theText) {
		CElement<?> c = cp5.getController(theControllerName);
		if (c == null) {
			return this;
		}
		map.put(c, theText);
	 ...
	 * @return Tooltip
	 */
	public Tooltip unregister(CElement<?> theController) {
	...
	/**
	 * with the default display, the width of the tooltip is set automatically, therefore setWidth() does not
	 * have any effect without changing the default display to a custom ControllerView.
	 * 
	 * @see com.hipposretribution.controlP5.elements.drawable.controllers.ControllerView
	 * @see com.hipposretribution.controlP5.drawable.Tooltip#setDisplay(ControllerView)
	 * @return Tooltip
	 */
	...
	/**
	 * @see com.hipposretribution.controlP5.drawable.Tooltip#setWidth(int)
	 * @param theHeight
	 * @return Tooltip
	 */
	...
	/**
	 * adds an offset to the position of the controller relative to the mouse cursor's position. default
	 * offset is (10,20)
	 * 
	 * @param theX
	 * @param theY
	...
	/**
	 * disables the Tooltip on a global level, when disabled, tooltip will not respond to any registered
	 * controller. to disable a tooltip for aparticular controller, used unregister(Controller)
	 * 
	 * @see com.hipposretribution.controlP5.drawable.Tooltip#unregister(CElement)
	 * @return Tooltip
	 */
	...
	class TooltipView implements ControllerView<CElement<?>> {

		@Override
		public void display(PApplet theApplet, CElement<?> theController) {
