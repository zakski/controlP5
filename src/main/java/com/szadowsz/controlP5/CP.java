...
public class CP {
	 ...
	 * 
	 * 
	 * <code>aArray</code> is a possibly-null array whose elements are primitives or objects; arrays of arrays
	 * are also valid, in which case <code>aArray</code> is rendered in a nested, recursive fashion.
	 * 
	...
	/*
	 * Base64 static methods to encode and decode bytes into a String and back
	 * 
	 * from http://examples.oreilly.com/javacrypt/files/oreilly/jonathan/util/
	...
	static public byte[] decodeBase64(String base64) {
		...
		for (int i = 0; i < base64.length(); i += 4) {
			int block = (getBase64Value(base64.charAt(i)) << 18) + (getBase64Value(base64.charAt(i + 1)) << 12)
					+ (getBase64Value(base64.charAt(i + 2)) << 6) + (getBase64Value(base64.charAt(i + 3)));
