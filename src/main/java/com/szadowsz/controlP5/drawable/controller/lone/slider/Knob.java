...
/**
 * A knob is a circular slider which can be used with a limited and unlimited range. Knobs come in 3 designs
 * LINE, ARC and ELIPSE and can be controller with both the mouse and the mouse wheel.
 * 
 * @example controllers/ControlP5knob
 */
public class Knob extends CElement<Knob> {
	...
	protected int _myDragDirection = ControlP5.HORIZONTAL;

	protected int viewStyle = ControlP5.LINE;
	 ...
	 * @exclude
	 */
	public Knob(ControlP5 theControlP5, CGroup<?> theParent, String theName, float theMin, float theMax,
			float theDefaultValue, int theX, int theY, int theWidth) {
		...
		_myUnit = (_myMax - _myMin) / ControlP5.TWO_PI;
		startAngle = ControlP5.HALF_PI + ControlP5.PI * 0.25f;
		angleRange = ControlP5.PI + ControlP5.HALF_PI;
		myAngle = startAngle;
		isConstrained = true;
		getCaptionLabel().setAlignment(AlignmentX.CENTER, AlignmentY.BOTTOM_OUTSIDE);
	...
	/**
	 * The start angle is a value between 0 and TWO_PI. By default the start angle is set to HALF_PI + PI *
	 * 0.25f
	 */
	...
	public Knob setDragDirection(int theValue) {
		if (theValue == ControlP5.HORIZONTAL) {
			_myDragDirection = ControlP5.HORIZONTAL;
		} else {
			_myDragDirection = ControlP5.VERTICAL;
		}
		return this;
	}
	...
	/**
	 * resolution is a sensitivity value when dragging a knob. the higher the value, the more sensitive the
	 * dragging.
	 */
	...
	public Knob updateInternalEvents(PApplet theApplet) {
		if (isMousePressed && !cp5.isAltDown()) {
			if (isActive) {
				float c = (_myDragDirection == ControlP5.HORIZONTAL) ? cp5.mouseDX() : cp5.mouseDY();
	...
	public void mousePressed() {
		float x = parent.getAbsolutePosition().x + position.x + _myRadius;
		float y = parent.getAbsolutePosition().y + position.y + _myRadius;
		if (PApplet.dist(x, y, cp5.mouseX(), cp5.mouseY()) < _myRadius) {
			isActive = true;
			if (PApplet.dist(x, y, cp5.mouseX(), cp5.mouseY()) > (_myRadius * 0.6)) {
				myAngle = (PApplet.atan2(cp5.mouseY() - y, cp5.mouseX() - x) - startAngle);
				if (myAngle < 0) {
					myAngle = ControlP5.TWO_PI + myAngle;
				}
				if (isConstrained) {
					myAngle %= ControlP5.TWO_PI;
				}
	...
	/**
	 * Changes the value of the knob when hovering and using the mouse wheel or the scroll function of a
	...
	/**
	 * set the display style of a knob. takes parameters Knob.LINE, Knob.ELLIPSE or Knob.ARC. default style is
	 * Knob.LINE
	 * 
	...
	@Override
	@ControlP5.Invisible
	public Knob updateDisplayMode(ControllerViewType theMode) {
		displayMode = theMode;
		switch (theMode) {
		case DEFAULT:
		case SPRITE:
		case IMAGE:
			controllerView = new KnobView();
			break;
		case CUSTOM:
		default:
			break;
	...
	class KnobView implements ControllerView<Knob> {
		...
		public void display(PApplet theApplet, Knob theController) {
			...
			if (isShowTickMarks()) {
				...
				theApplet.stroke(getColour().getForeground());
				theApplet.strokeWeight(getTickMarkWeight());
			...
			if (isLabelVisible()) {
				...
				_myValueLabel.setAlignment(AlignmentX.CENTER, AlignmentY.CENTER);
 ...
 * 
 * TODO dragging: add another option to control the knob. currently only linear dragging is implemented, add
 * circular dragging (as before) as well
 */
