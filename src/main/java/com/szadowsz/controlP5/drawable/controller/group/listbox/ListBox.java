package com.szadowsz.controlP5.drawable.controller.group.listbox;
...
public class ListBox extends ControlGroup<ListBox> {
	...
	public ListBox(ControlP5 theControlP5, CGroup<?> theGroup, String theName, int theX, int theY,
			int theW, int theH) {
		...
		_myScrollbar = new Slider(cp5, _myParent, theName + "Scroller", 0, 1, 1, _myWidth - _myScrollbarWidth, 0,
				_myScrollbarWidth, _myBackgroundHeight);
	...
	public ListBox setScrollbarVisible(boolean theScrollbarVisibleFlag) {
	...
	public ListBox updateInternalEvents(PApplet theApplet) {
		...
		boolean yy = cp5.mouseY() > getAbsolutePosition().y - 1
				&& cp5.mouseY() < getAbsolutePosition().y + _myBackgroundHeight;
	...
	protected ListBox addListButton(int theNum) {
		for (int i = 0; (i < theNum) && (buttons.size() < maxButtons); i++) {
			...
			Button b = new Button(cp5, this, _myName + "Button" + index, index, 0, index * (_myItemHeight + spacing),
	 ...
	 * 
	 * @see ListBox#removeItem(String,int)
	 ...
	 * 
	 * @see ListBox#addItem(String,int)
	...
	public void keyEvent(final KeyEvent theEvent) {
		...
		boolean b = (cp5.mouseX() > x && cp5.mouseX() < (x + _myWidth) && cp5.mouseY() > (y - getBarHeight()) && cp5
				.mouseY() < y + _myOriginalBackgroundHeight);
		if (b && isOpen()) {
			float step = (1.0f / items.size());
			if (cp5.isShiftDown()) {
				step *= 10;
	 ...
	 */
	@Override
	public ListBox setColour(CColor theColor) {
	 ...
	 */
	@Override
	public ListBox setColourActive(int theColor) {
	 ...
	 */
	@Override
	public ListBox setColourForeground(int theColor) {
	 ...
	 */
	@Override
	public ListBox setColourBackground(int theColor) {
	 ...
	 */
	@Override
	public ListBox setColourCaption(int theColor) {
	...
	@Override
	@ControlP5.Invisible
	public ListBox setColourValue(int theColor) {
