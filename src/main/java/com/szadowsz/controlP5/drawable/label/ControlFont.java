...
public class ControlFont {
	...
	/**
	 * set the RENDER_2X variable to true to double render text, this makes the font look bolder especially in
	 * OpenGL mode. use: ControlFont.RENDER_2X = true;
	 */
	...
	static private int checkFontSize(PFont theFont) {
		try {
			...
			System.out.println("ControlP5: could not find font-size details for font " + theFont.getName()
	...
	public int getSize() {
		/*
		 * quickfix http://code.google.com/p/controlp5/issues/detail?id=46 first check the pfont size then
		 * default back to size
		 */
		return size;
	}
	...
	public void adjust(PApplet theApplet, Label theLabel) {
		if (theLabel.hasChanged()) {
			...
			center = -((-top - bottom) / 2);
			height = theLabel.isMultiline() ? theLabel.getHeight() : (int) (theApplet.textAscent() + theApplet
					.textDescent());
			width = theLabel.isMultiline() ? theLabel.getWidth() : (int) theApplet.textWidth(theLabel
					.getFormattedText());
	...
	private void calculateHeight(PApplet theApplet, Label theLabel) {
		txt.clear();
		String myString = theLabel.getFormattedText();
	...
	public void draw(ControlP5 c, Label theLabel) {
		...
		c.papplet.textAlign(theLabel.textAlign.code);
		...
		if (theLabel.isMultiline()) {
			...
			c.papplet.fill(theLabel.getColor());
			c.papplet.textLeading(theLabel.getLineHeight());
			c.papplet.text(theLabel.getFormattedText(), 0, 0);
			if (RENDER_2X) {
				c.papplet.text(theLabel.getFormattedText(), 0, 0);
			}
		}
